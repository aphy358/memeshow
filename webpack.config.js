...
const walkEntries = app => {
	...
	// 遍历pages
	const { pages = [], subpackages = [], tabBar = {} } = require(resolve(appRoot, 'app.json'))
	...
	subpackages.forEach(it => walkSubPackageEntries(appRoot, it, entries, resolves))
...
const walkSubPackageEntries = (appRoot, package, entries, resolves, indent = '\t\t') => {
	const { root, pages = [] } = package
	const packageRoot = resolve(appRoot, root)
	// console.info(`${indent}subPackage => ${packageRoot}`)
	pages.forEach(it => walkPageEntries(resolve(packageRoot, it), entries, resolves, `${indent}\t`))
}
 ...
 * @param {string} indent 
 */
const walkComponent = (parentJsonPath, componentPath, entries, resolves, indent) => {
	...
	if (!path) {
		...
		if (path) {
			...
			const to = join(dist, relative(base, dir)).replace('node_modules', moduleDirAlias)
			// 替换配置的路径
			resolves.push({ parent: parentJsonPath, component: componentPath, resolveTo: join(relative(from, to), name) })
		}
	}
	if (!path) return
	...
	let entryName = isVendor ? relative(base, entryPath) : relative(src, entryPath)
	// 小程序会忽略node_modules目录，使用其它目录名
...
const resolveRules = resolves => {
	...
	const results = _.keys(resolvesByFiles).map(it => {
		return {
			...
			options: {
				multiple: resolvesByFiles[it].map(it => ({ search: it.component, replace: it.resolveTo }))
			}
		}
	})
	return results
...
const fullScriptPath = path => {
	...
	if (!ext) {
		const candidates = [resolve(dir, `${name}.js`), resolve(dir, name, 'index.js')]
		...
		module: {
			rules: [
				...
				},
				{
					...
					use: [{ loader: 'file-loader', options: { context: src, name: '[path][name].[ext]' } }],
					exclude: /node_modules/
				},
				{
					test: /.json$/,
