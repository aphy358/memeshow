...
var t = getApp(), tag = require("../../utils/html-tag");
Page({
    data: {
        title: "",
        cover: "",
    ...
    delPara: function(a) {
        var i = this, e = a.currentTarget.dataset.index;
        this.data.paraList[e].img && "" !== this.data.paraList[e].img && 1 === this.data.paraList.filter(function(t) {
            return t.img && "" !== t.img;
        }).length ? wx.showModal({
            title: "至少要保留一张图片",
            confirmColor: "#2F92FF",
            ...
            success: function(a) {
                if (a.confirm) {
                    var n = i.data.paraList, o = n[e].img;
                    n.splice(e, 1)
                    i.setData({
                      paraList: n
                    })
    ...
    pullUp: function(a) {
        ...
        if (0 !== i) {
            var e = t.globalData.editArticleParaList, n = e[i];
            e[i] = e[i - 1] 
            e[i - 1] = n; 
            this.setData({
                paraList: e
    ...
    pullDown: function(a) {
        var i = a.currentTarget.dataset.index, e = t.globalData.editArticleParaList;
        if (i !== e.length - 1) {
            var n = e[i];
            e[i] = e[i + 1]
            e[i + 1] = n
    ...
    expandAdd: function(t) {
      var a = t.currentTarget.dataset.index, i = null, e = null;
      this.animation = null;
      if(a== -1) {
    ...
    editPara: function(t) {
        if (!this.data.SomethingTapped) {
            var a = t.currentTarget.dataset, i = a.index, e = a.type || null;
            if (6 == e) {
              return wx.showToast({
                title: "暂不支持编辑小标题",
                icon: "none"
    ...
    addText: function(a) {
        if (this.data.paraList.length >= 150){
            ...
            var i = this.data.curIndex, e = this.data.paraList;
            this.closeAddPanel();
            var n = {
                img: "",
                text: "",
        ...
        if (this.data.paraList.length >= 150){ 
            ...
            if (e.length >= 100){ 
              wx.showModal({
                ...
                var n = 100 - e.length > 9 ? 9 : 100 - e.length, o = this.data.curIndex;
                wx.chooseImage({
                    count: n,
    ...
    addVideo: function(t) {
        ...
        this.data.paraList.length >= 150 ? wx.showModal({
            ...
            camera: "back",
            success: function(t) {
                if (t.duration < 3) wx.showModal({
                    title: "视频长度至少3秒，请重新选择",
                    confirmColor: "#2F92FF",
                    showCancel: !1
                }); else if (t.duration > 180) wx.showModal({
    ...
    changeCover: function() {
        t.globalData.editCover = this.data.cover, t.globalData.editArticleParaList = this.data.paraList, 
        wx.navigateTo({
            url: "./cover"
        });
    },
    removeDraft: function(t) {
        var a = this, i = t.currentTarget.dataset.draftKey;
        wx.showModal({
            title: "是否删除草稿？",
            content: "草稿删除后将无法恢复",
            cancelText: "不删除",
  ...
  onLoad: function (a) {
    ...
    if (a.temp_file_paths) {
      ...
      wx.getStorageSync("editImgTips") || setTimeout(function () {
        e.setData({
          isShowEditImgTips: !1
        });
      }, 1000)
    ...
    onShow: function() {
        ...
        }), this.isCloseAddPanel = !1, t.globalData.editArticleParaList) {
            var n = [];
            t.globalData.editArticleParaList.forEach(function(t, a) {
                (t.img && "" !== t.img || t.text && "" !== t.text || t.video_thumbnail && "" !== t.video_thumbnail) && n.push(t);
            }), this.setData({
                paraList: n
            }), t.globalData.editArticleParaList = n;
        }
    ...
    onUnload: function() {
        var a = this;
        if (this.data.removeDraftClicked) this.data.maskId && t.userInfoReadyCallback && t.userInfoReadyCallback(); else if (!this.data.forceReturn && 0 !== this.data.paraList.length && !this.data.postSuccess) if (this.data.maskId) if (this.data.draftKey) {
            var i = {
            ...
            }, e = this.data.draftKey, n = wx.getStorageSync(e);
            Object.assign(i, {
                time: e.substr(6)
            }), JSON.stringify(n) === JSON.stringify(i) || (wx.setStorageSync(e, i), wx.showModal({
                title: "退出编辑",
                content: "是否保存当前内容为草稿？",
                cancelText: "不保存",
                confirmText: "保存草稿",
                confirmColor: "#2F92FF",
                success: function(a) {
                    a.confirm && (t.getDraftsCallback && t.getDraftsCallback(), wx.switchTab({
                        url: "../me/me"
            ...
            if (!(t.globalData.originalArticleString === JSON.stringify(o))) {
                ...
                wx.setStorageSync(s, Object.assign(o, {
                    time: s.substr(6)
                })), wx.showModal({
                    title: "退出编辑",
                    content: "是否保存当前内容为草稿？",
            ...
            JSON.stringify(d) === JSON.stringify(l) || wx.showModal({
                title: "退出编辑",
                content: "是否保存当前内容为草稿？",
                cancelText: "不保存",
                confirmText: "保存草稿",
                confirmColor: "#2F92FF",
                success: function(a) {
                    a.confirm && (wx.setStorageSync(r, l), t.getDraftsCallback && t.getDraftsCallback(), 
                    wx.switchTab({
                        url: "../me/me"
                    }));
                }
            ...
            success: function(i) {
                if (i.confirm) {
                    ...
                    wx.setStorageSync(e, {
                        ...
                        time: e.substr(6)
                    }), t.getDraftsCallback && t.getDraftsCallback(), wx.switchTab({
                        url: "../me/me"
                    });
                }
            }
