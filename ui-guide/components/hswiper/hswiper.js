  ...
  properties: {
    ...
    items: {
      ...
      observer(newVal) {
        ...
        const { elements, activeElement, elementData, primaryKey } = this.data
        const activeItemIdx = activeElement.id
        // 这里只通过关键字段来查找元素所在数组的下标，是因为这个元素（其他属性）可能已经被修改了
        const activeDataIdx = newVal.findIndex(n => n[primaryKey] === elementData[activeItemIdx][primaryKey])
