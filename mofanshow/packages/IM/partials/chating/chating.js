...
let pageConfig = {
  data: {
    defaultUserLogo: '/packages/IM/images/default-icon.png',
  ...
  adInputConfirm() {
    let self = this
    if (self.data.adContent.length !== 0 && self.data.adImg.length !== 0 && self.data.adId.length !== 0) {
      let content = {
        type: 9,
        data: {
          content: self.data.adContent,
          img: self.data.adImg,
          id: self.data.adId
        }
      }
  ...
  testClick(e){
    ...
    let content = {
      type: 9,
      data: {
        content: '超级便宜的坚果 买三送一 时间不等人，明天就做爷爷了 9.9元/包',
        price: '9.9元/包',
        img: '',
        src: ''
      }
  ...
  adClickHandler(e) {
    let data = e.detail.data
    if (data === 'confirm') {
      if (this.data.adContent.length === 0) {
        showToast('text', '请输入内容')
      } else {
        this.adInputConfirm()
        this.setData({
  ...
  convertRawMessageListToRenderMessageArr(rawMsgList) {
    ...
    for(let time in rawMsgList) {
      ...
      if (rawMsg.type === 'custom' && JSON.parse(rawMsg['content'])['type'] === 1) {
        ...
        msgType = '贴图表情'
      } else if (rawMsg.type === 'custom' && JSON.parse(rawMsg['content'])['type'] === 9) {
        msgType = '自定义广告1号'
      } else {
        msgType = rawMsg.type
      }
      ...
      switch(msgType) {
        case '自定义广告1号': {
          let value = JSON.parse(rawMsg['content']).data
          specifiedObject = {
            ad: value
          }
          break
