...
Page({
  data: {
    article: {
       ...
       * [
       *   {
       *     // you need fill these fields when create a new node
       *     sort: integer,
       *     type: string ['base', 'video', 'vote', 'product', 'location']
       * 
       *     id: string,
       *     y: integer,
       *     height: integer,
   ...
   */
  handleReprint() {
    const instance = procedures.open("create-article/editors/reprint")
    const emitter = instance.asCaller()
    emitter.on('getArticle', data => {
      console.log(data)
      // todo
    })
  },
